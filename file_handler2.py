...
def process_profile_files():
    ...
    dates = [sdate + pd.DateOffset(days=d) for d in range((edate - sdate).days + 1)]
    df_list = []
    for date in dates:
        print(date)
        try:
            df = read_profile(str(date)[:10])
        except Exception as e:
            print(e)
            continue

        df = df[df.composite_ticker == 'SPY']
...
def process_constituent_files():
    ...
    name_list = []
    years = os.listdir(DIR_CONSTITUENTS)
    for year in years:
        ...
        df.to_pickle(f'data/constituents_{year}.pickle')

        with open("etfg_com_names.json", 'w') as f:
